...
width=0.1
plt.barh(labels, values, width, color = "blue")
plt.xlabel('Crime Incidents')
plt.ylabel('District')
plt.title('Crime Incident Reports Per District')
st.pyplot(plt)
st.text("Bar Chart of Crime Incident Reports Per District")
...
def chart(data_choice, type_chart):
    ...
    if data_choice == "Day of the week of the report" and type_chart == "Bar Chart":
    ...
    if data_choice == "Hour of the day of the report" and type_chart == "Bar Chart":
    ...
    if data_choice == "Month of the report" and type_chart == "Line Chart":
        incidents_by_month = df_bos.groupby(by = ['MONTH']).size().reset_index(name='count')
    ...
    if data_choice == "Day of the week of the report" and type_chart == "Line Chart":
        incidents_by_day = df_bos.groupby(by = ['DAY_OF_WEEK']).size().reset_index(name='count')
    ...
    if data_choice == "Hour of the day of the report" and type_chart == "Line Chart":
        incidents_by_hour = df_bos.groupby(by = ['HOUR']).size().reset_index(name='count')
    ...
    if data_choice == "Month of the report" and type_chart == "Pie Chart":
        incidents_by_month = df_bos.groupby(by = ["MONTH"]).size().reset_index(name='count')
    ...
    if data_choice == "Day of the week of the report" and type_chart == "Pie Chart":
        incidents_by_day = df_bos.groupby(by = ["DAY_OF_WEEK"]).size().reset_index(name='count')
    ...
    if data_choice == "Hour of the day of the report" and type_chart == "Pie Chart":
        incidents_by_hour = df_bos.groupby(by = ["HOUR"]).size().reset_index(name='count')
...
if select_pivot == "Yes":
    incidents_by_type = df_bos.pivot_table(index='OFFENSE_DESCRIPTION', columns='MONTH', values='INCIDENT_NUMBER', aggfunc='count')
